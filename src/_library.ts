...
type DynamicLibraryStructType =
{ readonly struct: Readonly<Array<DynamicLibraryType>> }
...
export type DynamicLibraryType =
  Exclude<keyof DynamicLibraryTypeMap, "void"> | DynamicLibraryStructType
export type DynamicLibraryResultType =
  keyof DynamicLibraryTypeMap | DynamicLibraryStructType
...
type ToNativeType<T extends DynamicLibraryType = DynamicLibraryType> = T extends
  DynamicLibraryStructType ? BufferSource
    : DynamicLibraryTypeMap[Exclude<T,DynamicLibraryStructType>];
type StaticForeignSymbolReturnType<T extends DynamicLibraryFunctionInterface> =
  T["result"] extends DynamicLibraryStructType ?
    Uint8Array:
...
type ToNativeParameterTypes<T extends readonly DynamicLibraryType[]> =
    ...
    [(T[number])[]] extends [T] ? ToNativeType<T[number]>[]
      ...
      : never;
type FromForeignFunction<T extends DynamicLibraryFunctionInterface> = 
