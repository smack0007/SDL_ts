...
export type SDL_CreateWindowFunc = (title: string, x: number, y: number, width: number, height: number, flags: number) => number;

export type SDL_DelayFunc = (time: number) => void;

export type SDL_DestroyWindowFunc = (window: number) => void;
